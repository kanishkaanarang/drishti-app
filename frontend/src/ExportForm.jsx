...
const initialState = {
  query_no: '', booking_party: '', shipper: '', iec_code: '', ad_code: '',
  consignee: '', commodity: '', moving_through: '', working_type: '', mode: '',
  shipment_terms: '', inv_no: '', inv_date: '', sb_be_no: '', sb_date: '',
  qty: '', unit: '', gross_weight: '', scheme: '', epcg_licence_no: '',
  dbk_amount: '', gst_amount: '', cha: '', carting_date: '', release_from_custom: '',
  cargo_handover_date: '', date_of_stuffing: '', tr_submission_date: '', remark: '',
  movement_type: '', cnt_no: '', size: '', railing_date: '', arrival_date_pol: '',
  transit_days_pol: '', si_submission_status: '', draft_approval_status: '',
  bl_awb_master: '', bl_awb_house: '', bl_awb_issue_date: '', shipping_airline: '',
  actual_forwarder: '', forwarder: '', vessel_flight: '', country_origin: '',
  origin_port: '', pol_poa: '', destination_port: '', country_destination: '',
  special_instructions: '', transshipment: '', transshipment_port_country: '',
  transshipment_port: '', transshipment_eta: '', transshipment_etd: '',
  etd: '', eta: '', transit_delivery_time: '', additional_remarks: '',
  insurance_required: '', insurance_amount: '', coo: '', coo_status: '',
  transport: '', transporter: '', bill_no: '', bill_date: '', original_recd_dt: '',
  bill_handover_dt: '', rate: '', detention: '', off_load_misc: '',
  kanta_charges: '', green_tax: '', total: '', tax_rate: '', tax_amount: '',
  invoice_value: '', remark_final: '', brokerage_remarks: '', brokerage_bill_no: '',
  billing_frc: '', frc_bill: '', billing_status: '', bill_number: '', job_status: ''
}
...
const Field = ({ label, name, value, onChange, type = 'text' }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
    ...
    <input
      ...
      style={{ padding: '8px 10px', border: '1px solid #ddd', borderRadius: 6, fontSize: 13, outline: 'none' }}
    />
  </div>
)
...
const Section = ({ title, children }) => (
  <div style={{ marginBottom: 32 }}>
    <h3 style={{ fontSize: 15, color: '#1A3C6E', borderBottom: '2px solid #1A3C6E', paddingBottom: 8, marginBottom: 16 }}>{title}</h3>
...
export default function ExportForm() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = { ...form, is_locked: true, created_by: user.id }
    ...
    numericFields.forEach(f => { if (payload[f] === '') payload[f] = null })
    dateFields.forEach(f => { if (payload[f] === '') payload[f] = null })

    const { error } = await supabase.from('export_shipments').insert([payload])
  ...
  if (success) return (
    <div style={{ textAlign: 'center', padding: 80 }}>
      <h2 style={{ color: '#2E7D32' }}>✓ Shipment saved and locked!</h2>
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, color: '#1A3C6E' }}>New Export Shipment</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#888' }}>All fields are saved and locked on submission. Only admin can edit after.</p>
        </div>
      ...
      <Section title="2. Invoice & Customs">
        ...
        <Field label="Advance Licence / EPCG No" name="epcg_licence_no" value={form.epcg_licence_no} onChange={handleChange} />
      ...
      {/* Section 3 — Dates & Movement */}
      <Section title="3. Cargo Dates & Movement">
        <Field label="Carting Date / Cargo Arrival" name="carting_date" value={form.carting_date} onChange={handleChange} type="date" />
        ...
        <Field label="Railing Date / Truck Movement" name="railing_date" value={form.railing_date} onChange={handleChange} type="date" />
        <Field label="Arrival Date at POL/FPOD" name="arrival_date_pol" value={form.arrival_date_pol} onChange={handleChange} type="date" />
      ...
      <Section title="4. Shipping Details">
        ...
        <Field label="Vessel / Flight Details" name="vessel_flight" value={form.vessel_flight} onChange={handleChange} />
      </Section>

      {/* Section 5 — Ports & Route */}
      <Section title="5. Ports & Route">
        ...
        <Field label="Transshipment Port ETA" name="transshipment_eta" value={form.transshipment_eta} onChange={handleChange} type="date" />
        <Field label="Transshipment Port ETD" name="transshipment_etd" value={form.transshipment_etd} onChange={handleChange} type="date" />
      ...
      <Section title="7. Transport & Billing">
        ...
        <Field label="Original Recd. Dt / Remarks" name="original_recd_dt" value={form.original_recd_dt} onChange={handleChange} />
        <Field label="Bill Handover Dt to Accounts" name="bill_handover_dt" value={form.bill_handover_dt} onChange={handleChange} type="date" />
        ...
        <Field label="Off Load / Extra Toll / Misc" name="off_load_misc" value={form.off_load_misc} onChange={handleChange} type="number" />
      ...
      <Section title="8. Brokerage & Job Status">
        ...
        <Field label="Job Status as on Date" name="job_status" value={form.job_status} onChange={handleChange} />
      </Section>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', paddingTop: 16, borderTop: '1px solid #eee' }}>
        <button onClick={() => navigate('/dashboard')}
          style={{ padding: '12px 28px', background: '#f5f5f5', border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}>
          Cancel
        </button>
        <button onClick={handleSubmit} disabled={loading}
          style={{ padding: '12px 32px', background: loading ? '#ccc' : '#1A3C6E', color: 'white', border: 'none', borderRadius: 6, cursor: loading ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 500 }}>
          {loading ? 'Saving...' : 'Submit & Lock Shipment'}
        </button>
      </div>
